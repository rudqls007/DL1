...
y = [54, 63, 57, 56, 58]                                        # 출력 데이터 (정답)

regr.fit(X, y)                                                  # 데이터 사용하여 훈련(학습)하기

# 직선의 기울기
coef = regr.coef_                                               # regr 모델의 기울기 속성값으로 얻음.
# 직선의 절편
intercept = regr.intercept_                                     # regr 모델의 intercept_ 속성값으로 연결
# 결과
...
plt.plot(X, y_pred, 'r:')                                      # 계산된 기울기와 y절편

fig.savefig("LinearRegression_result.png")

# 167인 순신이의 키를 넣어서 그 추정값을 출력하기
sin = [[167]]
result = regr.predict(sin)
print('순신이의 키가 {}cm 이므로 몸무게는 {}kg으로 추정이 된다.'.format(\
        sin, result.round(1)))